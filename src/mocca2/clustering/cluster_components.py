...
def cluster_components(
    ...
    if weights is None:
        ...
        assert len(compound_weights) == N, "The number of weights does not match the number of components"
    ...
    for cls in set(classes):
        ...
        elution_time = int(np.sum(elution_times*class_weights)/np.sum(class_weights))
...
def _assign_classes(adjacency: NDArray) -> List[int]:
    ...
    for new_idx, id in enumerate(ids):
        ...
        # DFS
        ids[new_idx] = next_id
        neighbouring = [idx for idx, adj in enumerate(adjacency[new_idx]) if adj and ids[idx] == -1]
        ...
        while len(neighbouring) != 0:
            ...
            new_neighbouring = [idx for idx, adj in enumerate(adjacency[curr_idx]) if adj and ids[idx] == -1]
